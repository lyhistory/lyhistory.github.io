...
+ 客户端 connect
    ...
    其中第一次握手client端主动向server端发起syn请求建立连接请求，server端收到后将与client端的连接设置为listen状态(半连接状态)并保存到半连接队列/backlog队列。  
...
[TCP三次握手最后一个ACK数据包丢失会发生什么？](https://mp.weixin.qq.com/s/M41ajl_d_dNjZULVRw-aLg)
...
现在回答问题：
[客户端通过TCP和UDP发三个包，服务端会收到多少个包](https://mp.weixin.qq.com/s/EpgrddQV2gM48ovskUhNSA)
...
当通过TCP发送数据时，即使调用了三次send()或sendto()函数，TCP并不会保证将这三次调用视为三个独立的“包”。TCP可能会将多次调用合并成一个或多个段（segment），也可能将单次调用分割成多个段，这取决于TCP的拥塞控制算法、窗口大小等因素。
因此，服务端接收到的TCP段数量可能少于、等于或多于客户端调用send()的次数。但是，服务端最终会接收到完整且按序的数据，即使这些数据被分割成了多个段。（发送数据调用send函数返回成功，并不代表数据已经发送到互联网，这一步只是表示数据从应用层拷贝到了内核中维护的发送缓冲区，至于内核什么时候发送就是由TCP/IP协议栈决定，应用程序是无法控制的，既不能控制它在什么时间发送，也不能控制它分为几个segment发送。）
...
[linux的TCP连接数量最大不能超过65535个，那服务器是如何应对百万千万的并发的？](https://mp.weixin.qq.com/s/abJpcRSbWW8HbrqVy6cvnQ)
...
https://sean22492249.medium.com/tcp-%E7%9A%84%E9%97%9C%E9%96%89%E5%8B%95%E4%BD%9C-1469750cd099
...
[TCP/IP协议栈的心跳、丢包重传、连接超时机制实例详解](https://mp.weixin.qq.com/s/PcE77v5jaDgMzD0e6i-hUQ)
