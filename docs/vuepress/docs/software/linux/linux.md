...
+ 解压直接使用 download tar file, and extract, then set env variable or persist it in bashrc bash_profile
...
+ 安装后找不到命令，因为安装的位置不在系统path上，可以link或者修改系统变量

  sudo ln -s /home/lyhistory/.local/bin/tldr tldr
  或者修改.bashrc export PATH=$PATH:$HOME/.local/bin（用户PATH ~/.bashrc， 全局PATH：/etc/profile或/root/.bashrc
