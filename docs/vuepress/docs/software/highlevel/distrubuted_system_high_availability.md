...
## 7*24
当要求一个系统支持“7乘24小时”不间断运行时，对于分布式系统意味着要平衡CAP:
+ Consistent 如何确保数据不丢不重最终一致
+ Availability 这确实是 高可用性（High Availability, HA）​ 最核心和最典型的目标
+ Partition tolerance 
+ 可靠性（Reliability）​ 
    指系统在长时间运行中不丢失数据、不出现故障、能够持续提供正确服务的能力。它更关注“数据会不会丢”“服务会不会断”“节点挂了是否影响整体”。可靠性是比一致性更广的范畴：一个系统可以最终一致但可靠（如Kafka），也可以强一致但不可靠（如单点数据库挂了就丢服务）。
